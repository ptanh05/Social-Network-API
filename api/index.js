...
function validate(schema) {
    return (req, res, next) => {
        ...
        if (!parsed.success)
            return err(res, 400, parsed.error.issues[0].message);
        next();
    };
}
...
async function issueAuthTokens(res, userId) {
...
app.post('/api/v1/auth/register', registerValidation, async (req, res) => {
    ...
    if (existingEmail)
        return err(res, 400, 'Email này đã được đăng ký. Vui lòng dùng email khác.');
    // Check duplicate username
    const existingUsername = await prisma.user.findUnique({ where: { username: body.username } });
    if (existingUsername)
        return err(res, 400, 'Username này đã tồn tại. Vui lòng chọn username khác.');
    try {
        ...
        const e2 = e;
        if (e2.code === 'P2002')
            return err(res, 400, 'Username hoặc Email đã tồn tại.');
...
app.get('/api/v1/auth/verify-email', async (req, res) => {
    ...
    if (!token)
        return res.redirect(`${FRONTEND_BASE_URL}/login?verified=missing`);
    ...
    if (!user)
        return res.redirect(`${FRONTEND_BASE_URL}/login?verified=invalid`);
...
app.delete('/api/v1/users/me', withAuth(async (req, res) => {
...
app.post('/api/v1/users/me/change-password', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.put('/api/v1/posts/:id', validate(updatePostSchema), withAuth(async (req, res) => {
    const u = getUser(req);
...
app.post('/api/v1/posts/:id/comments', validate(createCommentSchema), withAuth(async (req, res) => {
    const u = getUser(req);
    const postId = paramId(req, 'id');
...
// ─── Likes ───────────────────────────────────────────────────────
app.post('/api/v1/likes/posts/:id/like', withAuth(async (req, res) => {
    const u = getUser(req);
    const postId = paramId(req, 'id');
    const post = await prisma.post.findUnique({ where: { id: postId }, select: { id: true, authorId: true } });
    if (!post)
    ...
    try {
        ...
        if (e.code === 'P2002')
            return err(res, 400, 'Already liked');
        throw e;
    }
}));
app.delete('/api/v1/likes/posts/:id/like', withAuth(async (req, res) => {
    const u = getUser(req);
    const postId = paramId(req, 'id');
    const result = await prisma.like.deleteMany({ where: { userId: u.id, postId } });
    if (!result.count)
        return err(res, 404, 'Like not found');
    await prisma.post.update({ where: { id: postId }, data: { likesCount: { decrement: 1 } } });
    return ok(res, { liked: false });
}));
app.get('/api/v1/likes/posts/:id/status', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.post('/api/v1/follows/users/:id/follow', withAuth(async (req, res) => {
...
// ─── Preferences ─────────────────────────────────────────────────
app.get('/api/v1/preferences/users/me/preferences', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.put('/api/v1/preferences/users/me/preferences', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.get('/api/v1/notifications/unread-count', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.put('/api/v1/notifications/read-all', withAuth(async (req, res) => {
    const u = getUser(req);
...
// ─── Bookmarks ───────────────────────────────────────────────────
app.get('/api/v1/bookmarks', withAuth(async (req, res) => {
    const u = getUser(req);
    const cursor = safeCursor(req);
    const limit = safeLimit(req);
...
app.get('/api/v1/bookmarks/posts/:id/status', withAuth(async (req, res) => {
    const u = getUser(req);
...
app.post('/api/v1/bookmarks/posts/:id', withAuth(async (req, res) => {
    const u = getUser(req);
    const postId = paramId(req, 'id');
...
app.delete('/api/v1/bookmarks/posts/:id', withAuth(async (req, res) => {
    const u = getUser(req);
...
// ─── Reports ─────────────────────────────────────────────────────
app.post('/api/v1/reports', validate(createReportSchema), withAuth(async (req, res) => {
    const u = getUser(req);
    ...
    if (target_type === 'post') {
        ...
        if (!post)
            return err(res, 404, 'Post not found');
        if (post.authorId === u.id)
            return err(res, 400, 'Cannot report your own post');
    }
        ...
        if (comment.authorId === u.id)
            return err(res, 400, 'Cannot report your own comment');
    }
    else if (target_type === 'user') {
        if (target_id === u.id)
            return err(res, 400, 'Cannot report yourself');
...
app.get('/api/v1/reports', withAdmin(async (req, res) => {
    const cursor = typeof req.query.cursor === 'string' ? parseInt(req.query.cursor, 10) : undefined;
...
app.put('/api/v1/reports/:id', withAdmin(async (req, res) => {
    ...
    if (!report)
        return err(res, 404, 'Report not found');
