      ...
      RETURNING id, username, email, date_of_birth, is_admin, created_at`;
        ...
        if (e.code === '23505' || (e.fields && e.fields.constraint)) {
            ...
            if (constraint.includes('username'))
                return err(res, 400, 'Username already registered');
            if (constraint.includes('email'))
                return err(res, 400, 'Email already registered');
...
async function POST_create_post(req, res) {
    ...
    if (topic_ids?.length) {
        ...
        if (values)
            await sql`INSERT INTO post_topics (post_id, topic_id) VALUES ${sql.raw(values)} ON CONFLICT DO NOTHING`;
    }
