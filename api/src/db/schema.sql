...
-- ─── Denormalized counts helpers ────────────────────────────────────
-- Run these ONE TIME only to populate existing counts:
...
-- UPDATE users SET posts_count = (SELECT COUNT(*) FROM posts WHERE author_id = users.id);
