...
function safeCursor(req: AuthRequest): string | undefined { return typeof req.query.cursor === 'string' ? req.query.cursor : undefined; }
function safeLimit(req: AuthRequest): number { return Math.min(parseInt(typeof req.query.limit === 'string' ? req.query.limit : '20', 10), 50); }
...
router.get('/unread-count', withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
  const count = await prisma.notification.count({ where: { userId: u.id, isRead: false } });
  return ok(res, { count });
}));

router.put('/:id/read', withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
  await prisma.notification.updateMany({ where: { id: paramId(req, 'id'), userId: u.id }, data: { isRead: true } });
  return noContent(res);
}));

router.put('/read-all', withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
