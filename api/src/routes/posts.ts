...
function buildPostResponse(p: {
  id: number; content: string; authorId: number; createdAt: Date; updatedAt: Date; likesCount: number; commentsCount: number;
  author: { id: number; username: string; email: string; dateOfBirth: Date | null; isAdmin: boolean; createdAt: Date };
  topics: Array<{ topic: { id: number; name: string; description: string | null } }>;
}, feed_score = 0) {
  return {
...
router.get('/feed', withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
...
router.get('/explore', withAuth(async (req: AuthRequest, res: Response) => {
...
router.get('/search', withAuth(async (req: AuthRequest, res: Response) => {
...
router.post('/', validate(createPostSchema), withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
  ...
  await Promise.all(hashtags.map((name) =>
...
router.put('/:id', validate(updatePostSchema), withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
...
router.delete('/:id', withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
  const postId = paramId(req, 'id');
  const existing = await prisma.post.findUnique({ where: { id: postId } });
  if (!existing) return err(res, 404, 'Post not found');
...
router.get('/:id/comments', withAuth(async (req: AuthRequest, res: Response) => {
  const postId = paramId(req, 'id');
...
router.post('/:id/comments', validate(createCommentSchema), withAuth(async (req: AuthRequest, res: Response) => {
  const u = getUser(req);
  const postId = paramId(req, 'id');
...
router.get('/trending/hashtags', withAuth(async (_req: AuthRequest, res: Response) => {
